...
| `CalculiXUMATTarget`  | ✓ elastic | `<Model>_umat.cpp`: `extern "C" umat_user_` (CalculiX native ABI) linked INTO ccx; tmech `abq_std` Voigt boundary. CI: ABI gate vs. an independent oracle; manual: real single-element `ccx` run (`examples/calculix/`) |
| `CalculiXExternalTarget` | ✓ elastic | `<Model>_ext.cpp` → `lib<MODEL>.so`: CalculiX external behaviour loaded at RUNTIME via `dlopen` (deck `NAME=@<MODEL>_NCG_UMAT`) — **no ccx recompile per material**. Same `abq_std` boundary; per-thread evaluator. CI ABI gate + manual `ccx` run (`examples/calculix/`) |
