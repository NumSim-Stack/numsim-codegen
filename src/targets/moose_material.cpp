...
auto emit_source(ConstitutiveModel const &model, std::string const &app_name)
    -> std::string {
  // Emit the Layer-2 function first so the Eigen-include decision tracks the
  ...
  // synthesized coupling) → a missing `<Eigen/Dense>` for code that uses it.
  // Keying on the `Eigen::` usage in the body cannot drift.
  ...
  os << "#include <cmath>\n";
  // Phase 3b-2b: a coupled local-Newton system's dense solve uses Eigen. MOOSE
  // builds Eigen's headers under -Werror, so use libMesh's warning-suppression
  // idiom (the bare `<Eigen/Dense>` would risk build-breaking warnings inside
  // Eigen). Requires libMesh built with Eigen (the default).
  if (needs_eigen) {
