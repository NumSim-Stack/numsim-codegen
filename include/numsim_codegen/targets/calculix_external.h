...
//
// The external STANDARD interface passes NATIVE quantities under Abaqus-flavored
// names — STRAN1=emec (tensorial), STRESS=stre, DDSDDE=stiff(21) packed — so the
// boundary is a tmech `abq_std` Voigt adaptor + column-major stiff(21) packing
// (shared in calculix_boundary.h), wrapped in CalculiX's external ABI (the
// `calculixptr` signature) and organised as a plugin lifecycle:
