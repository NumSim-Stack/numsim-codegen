...
| **2.6** | MOOSE backend wiring: `declareProperty` (current) / `getMaterialPropertyOld` (old), `dt`→`_dt`, `initQpStatefulProperties`, compute call via `canonical_arguments` (issue #77). Requires `enable_local_newton()` (MOOSE solves internally); constant initial values only. | `MooseMaterialTarget` | ✓ landed |
...
| **3b-1** | `AlgorithmicTangentPass` (consistent tangent dσ/dε; opt-in via `add_algorithmic_tangent`). Explicit term `∂σ/∂ε` via `cas::diff(tensor,tensor)` ships now; with the current scalar-residual machinery `dx/dε ≡ 0` so this is the exact tangent (elastic / strain-independent state). Symmetric-role rank-2 inputs (`roles::Strain`) carry a symmetric `tensor_space`, so the tangent is minor-symmetric (`2μ·½(otimesu+otimesl)`). | After Newton lowering, before CodeEmitPass; advertises `tangent-emitted` | ✓ landed |
| **3b-1 (coupled)** | Implicit correction `∂σ/∂x·dx/dε` for strain-coupled internal variables. Stub seam `detail::diff_tensor_wrt_scalar` (numsim-cas#275) + a t2s-typed residual in `NewtonSegment`; macro-guarded by `NUMSIM_CODEGEN_HAVE_DIFF_TENSOR_WRT_SCALAR`. | `AlgorithmicTangentPass::run` correction block | blocked on numsim-cas#275 |
| **3b-2b** | Coupled vector Newton: `LocalNewtonLoweringPass` auto-detects coupling (union-find over residual scalar-leaves), solving each mutually-referencing group as ONE dense N×N Newton loop; uncoupled equations stay 1×1 (byte-identical). Dense solve via a `LinearAlgebraEmitter` (Eigen default + Armadillo, injected per-target); include gated on the emitter's `usage_marker()`. Scalar unknowns only. | `LocalNewtonLoweringPass` / `render_compute_function` / `code_emit/linear_algebra_emitter.h` | ✓ landed |
| **3b-2 (KT/multi-surface)** | `KuhnTuckerLoweringPass` (Fischer-Burmeister NCP) + multi-surface + tensor-unknown systems; rank-4 `tensor_inv` (numsim-cas#276 / #43) + `diff(t2s,scalar)` (numsim-cas#285) | New pass before LocalJacobian; builds on the 3b-2b coupled solver | future |
| **5 (tangent wiring)** | MOOSE consistent-tangent → framework `_Jacobian_mult` slot. `canonical_arguments` carries the tangent identity as `ArgSpec::Role::TensorTangentOutput` (derived structurally from `tangents()`, so the call-site and signature stay in lockstep pre/post pass); the MOOSE backend emits the `_Jacobian_mult` member / `declareProperty<RankFourTensor>("Jacobian_mult")` / write adaptor. One tangent per recipe on MOOSE; standalone emits it as a plain rank-4 out-param. | `recipe_render.h` ArgSpec / `recipe.h` canonical_arguments / `moose_material.cpp` | ✓ landed |
